...
export const CountrySelect: React.FC<Props> = ({
  ...
  return (
    <Listbox value={value} onChange={onChange}>
      {({ open }) => (
        <>
          <Listbox.Label className="block text-sm font-medium leading-6 text-secondary-900">
            {label}
          </Listbox.Label>

          <div className="relative mt-2">
            <Listbox.Button className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-secondary-900 shadow-sm ring-1 ring-inset ring-secondary-300 focus:outline-none focus:ring-2 focus:ring-primary-600 sm:text-sm sm:leading-6">
              ...
              <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                <UpDownIcon className="h-5 w-5 text-secondary-400" />
              </span>
            </Listbox.Button>
